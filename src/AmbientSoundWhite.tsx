...
export default function AmbientSoundWhite() {
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 left-4 z-50 w-12 h-12 rounded-full flex items-center justify-center
          ${muted ? "bg-gray-200" : "bg-white animate-glow"}
          shadow-lg transition-all duration-300`}
        aria-label="Activer ou couper le son"
      >
        <div className={`relative w-6 h-6 ${muted ? "" : "animate-bouncewing"}`}>
          {!muted && (
            <span className="absolute inset-0 rounded-full border-2 border-white animate-ping-fast opacity-50" />
          )}
          <div
